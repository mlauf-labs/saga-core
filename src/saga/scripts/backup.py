"""Backup/export script (``saga-backup``), FR-28..31.

Pages through the REST export endpoint and writes, per document, into a directory
derived from the document's primary folder path: the original binary, the converted
``.md`` text, and a ``*.metadata.json`` sidecar.

Usage:
    saga-backup --base-url http://localhost:8000 --token <token> --out ./backup
"""
...
async def _write_document(
    ...
    rel_dir = out_dir / Path(*backup_relative_dir(document.get("primary_folder_path", [])).parts)
...
def main() -> None:  # pragma: no cover - thin CLI wrapper around run_backup
    parser = argparse.ArgumentParser(description="Back up all Saga documents to disk.")
