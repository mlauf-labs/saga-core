...
from saga.api.schemas import ErrorResponse
from saga.core.errors import (
    AuthError,
    ConflictError,
    ConversionError,
    NotFoundError,
...
_STATUS_MAP: dict[type[SagaError], int] = {
...
def _status_for(exc: SagaError) -> int:
...
def register_exception_handlers(app: FastAPI) -> None:
    """Register handlers that turn ``SagaError`` into JSON error responses."""

    async def handle_saga_error(_: Request, exc: Exception) -> JSONResponse:
        assert isinstance(exc, SagaError)
    ...
    app.add_exception_handler(SagaError, handle_saga_error)
