...
async def _record(
    sink: EventSink, *, actor: str, summary: str, document_id: str | None, details: dict[str, Any]
) -> None:
    now = datetime.now(UTC)
    event = Event(
        event_id=uuid.uuid4().hex,
        category=EventCategory.AUDIT,
        event_type=EventType.EVENT_CURATED,
...
async def delete_event(
    store: EventMutationStore, sink: EventSink, event_id: str, *, actor: str = "agent"
) -> None:
...
async def update_event(
    ...
    *,
    ...
    updated = await store.update_event(
        event_id, summary=summary, occurred_at=occurred_at, confidence=confidence, details=details
    )
    if updated is None:
    ...
    await _record(
        sink,
        actor=actor,
        summary=f"Updated event {event_id}",
